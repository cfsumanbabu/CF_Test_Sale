...
contract CFS is ERC20Token {
  ...
  string public  name     = "CrowdForce Test Sale Token";
  ...
  uint public tokensPerEth = 200 * E6; // rate 
  ...
  uint public  TOKEN_SUPPLY_TOTAL = 200 * E6 * E6; // 400 mm tokens
  uint public  TOKEN_SUPPLY_ICO   = 160 * E6 * E6; // 320 mm tokens
  uint public  TOKEN_SUPPLY_CEO   =  40 * E6 * E6; //  80 mm tokens
  ...
  function CFS() {
    require( TOKEN_SUPPLY_ICO + TOKEN_SUPPLY_CEO == TOKEN_SUPPLY_TOTAL );
    wallet = owner;
    adminWallet = owner;
  }
  ...
  function icoThresholdReached()  returns (bool thresholdReached) {
     if (tokensIssuedIco < MIN_FUNDING_GOAL)
      return false;
  ...
  function mintMarketing(address _participant, uint _tokens) onlyOwner {
    // check amount
    require( _tokens <= TOKEN_SUPPLY_CEO.sub(tokensIssuedCeo) );
    
    // update balances
    balances[_participant] = balances[_participant].add(_tokens);
    tokensIssuedCeo        = tokensIssuedCeo.add(_tokens);
